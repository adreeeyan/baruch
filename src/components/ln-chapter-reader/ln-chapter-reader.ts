...
export class LnChapterReader implements OnInit {
  ...
  nextPage() {
    ...
    // override the skipper value
    // check if there is a truncated text in the bottom of the screen
    // 1. traverse all the paragraphs
    // 2. check if paragraph "top" is between 0 and skipper value and bottom is greater than skipper value
    // 3. get that paragraph's top value and deduct it from skippers value
    // 4. set variable = skippers value - value from number 3 + navHeight
    // 5. override skipper value with that variable
    // var holder: HTMLElement = this.contentHolder.nativeElement;
    // var lines = holder.getElementsByTagName("p");
    // for (var i = 0; i < lines.length; i++) {
    //   var line = lines[i];
    //   var lineRect = line.getBoundingClientRect();
    //   var lineTop = lineRect.top;
    //   var lineBottom = lineRect.bottom;
    //   var calculatedScrollTop = lineBottom - this.pageHolder.scrollTop;
    //   if (lineTop >= 0 && lineTop <= this.skipper && lineBottom > this.skipper && calculatedScrollTop < this.skipper) {
    //     console.log("halaaaaaaaaaaa", line);
    //     var skipTop = this.skipper - lineTop;
    //     var newSkipper = this.skipper - skipTop + this.navBarHeight;
    //     this.pageHolder.scrollTop += newSkipper;
    //     return;
    //   }
    // }

    this.skipper = this.origSkipper - this.fontSize;
    this.pageHolder.scrollTop += this.skipper;
  }
